...
<?php
	...
	$flag = false;
		if(isset($_POST['lastname']) && isset($_POST['name']) && isset($_POST['otchestvo']) && isset($_POST['day']) && isset($_POST['month']) && isset($_POST['year']) && isset($_POST['pol']) && isset($_POST['login']) && isset($_POST['password']) && isset($_POST['pass']) && isset($_POST['mail']) && isset($_POST['about']))
		{
			if(!preg_match( "/(\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,6})/", $_POST['mail']))
			{
				$flag = true;
			...
			{
				$flag = true;
				$lastnameinv = '<br> Введите фамилию кирилицей';
			}
			if (!preg_match("/^[А-Яа-я]+$/u", $_POST['otchestvo']))
			{
				$flag = true;
				$otchestvinv = '<br> Введите отчество кирилицей';
			}
			if (!preg_match("/^[А-Яа-я]+$/u", $_POST['name']))
			{
				$flag = true;
...
<form method="post" class="login">
 	<body>			
			<form method=post> 
				<p>Фамилия</p><input type="text" name="lastname" value="<?php if(isset($_POST['lastname'])) echo $_POST['lastname'];?>"><span><?php if(isset($emplastname)) echo $emplastname;?></span><span><?php if(isset($lastnameinv)) echo $lastnameinv;?></span>
				<p>Имя:</p><input type="text" name="name" value="<?php if (isset($_POST['name'])) echo $_POST['name'];?>"><span><?php if(isset($empname)) echo $empname;?></span><span><?php if(isset($nameinv)) echo $nameinv;?></span>
				<p>Отчество</p><input type="text" name="otchestvo" value="<?php if ( isset ($_POST['otchestvo'] ) ) echo $_POST['otchestvo'];?>"> <span><?php if(isset($otchestv)) echo $otchestv;?> </span><span><?php if(isset($otchestvinv)) echo $otchestvinv;?></span>
				<p>Пол:</p>
				...
				<p>Логин:</p><input type="text" name="login" value="<?php if ( isset ($_POST['login'] ) ) echo $_POST['login']?>"><span ><?php if ( isset ($emptLogin) ) echo $emptlogin?></span><span><?php if(isset($samelog)) echo $samelog;?></span><span><?php if(isset($logininc)) echo $logininc;?></span>
				<p>Пароль:</p><input type="password" name="password" value="<?php if ( isset ($_POST['password'] ) ) echo $_POST['password']?>"><span ><?php if ( isset ($emppwd) ) echo $emppwd?></span><span><?php if(isset($pwdinc)) echo $pwdinc;?></span>
